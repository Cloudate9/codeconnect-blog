...
<body class="bg-zinc-800">
	...
	<div class="mb-8 grid place-items-center bg-zinc-700 p-4 text-center lg:my-8">
	...
	{:else}
		<div class="grid-rows m-10 grid">
			...
			{#each posts as post (post.id)}
				{#if post === posts.at(-1)}
					<!-- If it is the last element, add an intersection observer to load more data when we read the last one -->
					<IntersectionObserver
						once
						element={lastPostFectched}
						on:intersect={() => fetchAndStorePosts()}
					>
						<!-- Bg colour should be zinc-900 if there is no hover, matching the bg -->
						<div bind:this={lastPostFectched} class="rounded-md p-4 duration-300 hover:bg-zinc-700">
							<a href="/post/{post.id}">
